...
export function UseTemplateModal({ template, onClose }: { template: UseTemplateTarget | null; onClose: () => void }) {
    ...
    useEffect(() => { setGroom(''); setBride(''); setError(null); }, [template?.key]);

    const C = dict({
        bm: { title: 'Cipta kad', groom: 'Nama pengantin lelaki', bride: 'Nama pengantin perempuan', groomPh: 'cth. Adam', bridePh: 'cth. Hawa', create: 'Cipta Kad', creating: 'Mencipta…', cancel: 'Batal', failed: 'Kad belum berjaya dicipta. Sila cuba lagi.', hint: 'Setiap kad untuk satu majlis. Masukkan nama pasangan untuk mula.' },
        en: { title: 'Create a card', groom: "Groom's name", bride: "Bride's name", groomPh: 'e.g. Adam', bridePh: 'e.g. Hawa', create: 'Create card', creating: 'Creating…', cancel: 'Cancel', failed: 'Failed to create card. Please try again.', hint: "Each card is for one event. Enter the couple's names to start." },
        zh: { title: '创建请柬', groom: '男方姓名', bride: '女方姓名', groomPh: '例如 Adam', bridePh: '例如 Hawa', create: '创建请柬', creating: '创建中…', cancel: '取消', failed: '请柬创建失败，请重试。', hint: '每张请柬用于一个婚礼。输入新人姓名即可开始。' },
    }, lang);
    ...
    async function create() {
        ...
        setError(null);
        try {
            const r = await api.post<{ id: string }>('/invitations', { template_key: tpl.key, groom_name: groom, bride_name: bride });
            ...
            <div className="panel" style={{ maxWidth: 400, width: '100%' }} onClick={(e) => e.stopPropagation()}>
                ...
                <div className="row" style={{ gap: 12, marginBottom: 16, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                        ...
                        <p className="muted" style={{ fontSize: 12.5, margin: '4px 0 0', lineHeight: 1.5 }}>{C.hint}</p>
                    </div>
                </div>
                ...
                <div className="row" style={{ gap: 10, marginTop: 8 }}>
                    ...
                    <button type="button" className="btn btn-primary grow" onClick={create} disabled={creating || !groom.trim() || !bride.trim()}>
